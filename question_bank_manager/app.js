...
async function saveApiKey() {
  ...
  // Test key & fetch model list
  const models = await fetchAvailableGeminiModels(geminiApiKey);
  if (models.length > 0) {
    showToast('success', `Gemini API Key saved! Found ${models.length} active models.`);
  } else {
...
// When opening API key modal, auto-load models
...
function processAiImageFile(file) {
  ...
  reader.onload = (e) => {
    ...
    // Optimize screenshot size via HTML5 Canvas for 10x faster AI upload & processing
    compressImageForAi(rawDataUrl, 1200, 0.85, (compressedDataUrl, mime, base64) => {
      currentAiImageBase64 = base64;
      currentAiImageMime = mime;

      aiPreviewImg.src = compressedDataUrl;
      aiFileName.textContent = `${file.name} (Optimized for speed)`;
      aiUploadPlaceholder.style.display = 'none';
      aiPreviewBox.style.display = 'block';

      showToast('success', 'Screenshot optimized & ready!');
    });
  };
...
`;
    ...
    // Query Google's ListModels endpoint to get EXACT valid model names for this key
    let modelsToTry = [];
    try {
      btnRunAi.innerHTML = `
        <svg class="spinner" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="18" height="18" style="animation: spin 1s linear infinite;">
        ...
        Finding active models...
      `;
      const listRes = await fetch(`https://generativelanguage.googleapis.com/v1beta/models?key=${geminiApiKey}`);
      if (listRes.ok) {
        const listData = await listRes.json();
        const validFromApi = (listData.models || [])
          .filter(m => m.supportedGenerationMethods && m.supportedGenerationMethods.includes('generateContent'))
          .map(m => m.name.replace('models/', ''));
        if (validFromApi.length > 0) {
          modelsToTry = validFromApi;
        }
      }
    } catch (e) {
      console.warn('ListModels query failed, fallback to defaults:', e);
    }

    if (modelsToTry.length === 0) {
      modelsToTry = ['gemini-1.5-flash', 'gemini-1.5-pro', 'gemini-2.0-flash-exp'];
    }

    // Prioritize cached working model if present
    ...
    for (const modelName of modelsToTry) {
      try {
        ...
        // 8-second timeout per request to prevent hanging indefinitely
        ...
        if (response.ok) {
          data = resData;
          console.log(`Successfully received AI response from ${modelName}`);
    ...
    if (!data) {
      throw new Error(`Gemini API call failed. Error: ${lastError}`);
    }
