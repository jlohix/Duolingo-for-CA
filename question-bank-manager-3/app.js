/* ============================================
   Question Bank Manager 3 — Application Logic
   With RFC-4180 Multiline CSV Parser & Auto Drag Path Link Generator
   ============================================ */
...
function bindEvents() {
  ...
  // AI Drag & Drop with Path Extraction (Requirement 2)
  ...
  // Image Upload Form Drag & Drop (Requirement 2)
...
// ============================================
// Drag & Drop Traversal for Section Paths (Requirement 2)
// ============================================
...
function cleanDraggedRelativePath(rawPath) {
  ...
  // 1. If path contains 'question-bank/', extract everything AFTER 'question-bank/'
  const qbIdx = clean.toLowerCase().indexOf('question-bank/');
  if (qbIdx !== -1) {
    clean = clean.substring(qbIdx + 'question-bank/'.length);
  } else {
    // 2. Extract parent folder + filename if parent is not a system directory
  ...
  // Strip redundant leading repository names if present
...
function saveToStorage() {
  try {
    localStorage.setItem('qb3_questions', JSON.stringify(questions));
    localStorage.setItem('qb3_local_images', JSON.stringify(localImageStore));
  } catch (e) {
...
function loadFromStorage() {
  try {
    const stored = localStorage.getItem('qb3_questions');
    if (stored) questions = JSON.parse(stored);

    const storedImages = localStorage.getItem('qb3_local_images');
...
// ============================================
// Image Upload for Question Form with Drag Path (Requirement 2)
// ============================================
...
// ============================================
// CSV Import & Export — RFC 4180 State Machine Parser (Requirement 1 Fix)
// ============================================
 ...
 * Character-by-character parser handling double quotes, newlines, and colons safely within fields.
 */
...
function generateCSVString() {
  ...
  questions.forEach(q => {
    const row = headers.map(h => {
      ...
      // Wrap cell in double quotes if it contains commas, quotes, newlines, or colons
