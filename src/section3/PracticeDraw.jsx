...
export function BatteryV({ x, y1, y2 }) {
  const mid = (y1 + y2) / 2;
  const gap = 8;
...
export function ResistorH({ x1, x2, y }) {
  const lead = 10;
  const a = x1 + lead;
  const b = x2 - lead;
  const w = (b - a) / 6;
...
export function CapacitorH({ x1, x2, y }) {
  const mid = (x1 + x2) / 2;
  const gap = 7;
  const left = mid - gap;
  const right = mid + gap;
...
export function InductorH({ x1, x2, y }) {
  const lead = 10;
  const a = x1 + lead;
...
export function ResistorV({ x, y1, y2 }) {
  const lead = 10;
  const a = y1 + lead;
  const b = y2 - lead;
  const w = (b - a) / 6;
...
export function CurrentArrow({ x, y, label, weak }) {
  return (
    ...
    >
      <g className={open ? "walk-dim" : undefined}>
        ...
        <text x={Xb} y={198} textAnchor="middle" className="walk-part">
          {open ? "Battery out" : vsValue || sourceLabel}
        </text>
      </g>
...
export function ResponseGraph({ kind, yTop, yBot, yLabel }) {
